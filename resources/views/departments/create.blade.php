<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-semibold leading-tight text-[var(--portal-text-primary)]">{{ __('Create Department') }}</h2>
    </x-slot>
        ...
        <div class="rounded-none border border-[var(--portal-border)] bg-[var(--portal-surface)] p-6">
            <form method="POST" action="{{ route('departments.store') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="4" class="mt-2 block w-full rounded-none border border-[var(--portal-border)] bg-[var(--portal-bg)] text-[var(--portal-text-primary)] placeholder:text-[var(--portal-text-secondary)] focus:border-[var(--portal-primary)] focus:ring-[var(--portal-primary)]">{{ old('description') }}</textarea>
                ...
                <div class="flex items-center justify-end gap-2">
                    ...
                    <x-primary-button><i class="bi bi-check-lg mr-2"></i>{{ __('Save Department') }}</x-primary-button>
                </div>
            </form>
        </div>
    </div>
